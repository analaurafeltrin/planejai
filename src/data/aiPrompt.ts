...
/*
 * aiService (data) — buildAIPrompt
 
 * Monta o prompt enviado à IA (Gemini) para gerar o diagnóstico financeiro personalizado.
 
 * RESPONSE_SCHEMA: define a estrutura JSON esperada na resposta, com os campos:
 *   feasibility, diagnosis, suggestions, extraIncome, investment, motivation
 
 * buildAIPrompt: recebe um SimulationRecord, calcula monthlySavings e
 *   monthlySavingsNeeded, e retorna um prompt instrucional que:
 *   - Define o papel da IA (educador financeiro, linguagem didática, 2ª pessoa)
 *   - Injeta os dados da simulação com os valores calculados
 *   - Exige resposta em JSON puro (sem markdown), seguindo o RESPONSE_SCHEMA
 *   - Define as regras de preenchimento e os critérios de feasibility.status:
 *       viable (saldo ≥ 0) | needs_adjustment (negativo até 20%) | unfeasible (negativo > 20%)
 */
